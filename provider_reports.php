...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'submit_report') {
        ...
        // Build a structured description for the admin's ticket view
        ...
        $ticket_type_map = [
            'Equipment Issue'        => 'Technical Problem',
            'Supply Request'         => 'General Inquiry',
            'Patient Concern'        => 'General Inquiry',
            'Schedule Conflict'      => 'Appointment Issue',
            'Incident Report'        => 'General Inquiry',
            'Maintenance Request'    => 'Technical Problem',
            'Other'                  => 'General Inquiry',
        ];

        $ticket_type = $ticket_type_map[$report_type] ?? 'General Inquiry';
    ...
    // --- READ: Provider's submitted reports ---
    ...
    // --- READ: Provider's appointment stats for quick summary ---
    $stats_stmt = $pdo->prepare("SELECT
                                    COUNT(*) as total,
                                    SUM(CASE WHEN status = 'Pending' THEN 1 ELSE 0 END) as pending,
                                    SUM(CASE WHEN status = 'Confirmed' THEN 1 ELSE 0 END) as confirmed,
                                    SUM(CASE WHEN status = 'No-Show' THEN 1 ELSE 0 END) as noshow,
                                    SUM(CASE WHEN DATE(s.date) = CURDATE() THEN 1 ELSE 0 END) as today
                                 FROM appointments a
...
<head>
    ...
    <style>
        ...
        .report-type-btn:hover, .report-type-btn.selected {
            border-color: var(--citu-blue);
            color: var(--citu-blue);
            background: #EEF2FF;
            font-weight: 600;
        }

        .report-type-btn .ricon { font-size: 1.4rem; display: block; margin-bottom: 4px; }
        ...
        <div>
            ...
            <div class="card" style="margin-bottom:20px;">
                ...
                <div class="summary-grid">
                    <div class="summary-tile">
                        <div class="s-num"><?php echo $appt_stats['today'] ?? 0; ?></div>
                        <div class="s-label">Today's Patients</div>
                    </div>
                    <div class="summary-tile">
                        <div class="s-num"><?php echo $appt_stats['pending'] ?? 0; ?></div>
                        <div class="s-label">Pending</div>
                    </div>
                    <div class="summary-tile">
                        <div class="s-num"><?php echo $appt_stats['noshow'] ?? 0; ?></div>
                        <div class="s-label">No-Shows Total</div>
                    </div>
                    <div class="summary-tile">
                        <div class="s-num"><?php echo $appt_stats['total'] ?? 0; ?></div>
                        <div class="s-label">All Appointments</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <form method="POST" id="report-form">
                    ...
                    <input type="hidden" name="report_type" id="hidden_report_type" required>
                    ...
                    <div class="input-group">
                        ...
                        <div class="report-type-grid">
                            <button type="button" class="report-type-btn" onclick="selectType('Equipment Issue', this)">
                                <span class="ricon">🔧</span>Equipment Issue
                            </button>
                            <button type="button" class="report-type-btn" onclick="selectType('Supply Request', this)">
                                <span class="ricon">📦</span>Supply Request
                            </button>
                            <button type="button" class="report-type-btn" onclick="selectType('Patient Concern', this)">
                                <span class="ricon">🧑‍⚕️</span>Patient Concern
                            </button>
                            <button type="button" class="report-type-btn" onclick="selectType('Schedule Conflict', this)">
                                <span class="ricon">📅</span>Schedule Conflict
                            </button>
                            <button type="button" class="report-type-btn" onclick="selectType('Incident Report', this)">
                                <span class="ricon">⚠️</span>Incident Report
                            </button>
                            <button type="button" class="report-type-btn" onclick="selectType('Other', this)">
                                <span class="ricon">📝</span>Other
                            </button>
                        </div>
                    </div>
        ...
        <div class="card">
            ...
            <?php if (count($reports) > 0): ?>
                <?php foreach ($reports as $r): ?>
                    <?php
                        // Extract the subject line from structured description
                        ...
                        $priority_label = $pri_match[1] ?? 'Medium';
...
function selectType(type, btn) {
    ...
    document.getElementById('hidden_report_type').value = type;
}
...
function validateReport() {
    const type = document.getElementById('hidden_report_type').value;
    if (!type) {
